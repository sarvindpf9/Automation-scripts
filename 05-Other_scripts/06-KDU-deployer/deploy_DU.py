...
required_vars = ['DOMAIN', 'BORK', 'EMAIL', 'BORK_TOKEN', 'SHORTNAME', 'AIM', 'PASSWORD', 'REGION', 'SPOT_ORGANIZATION_NAME', 'CHART_NAME']
...
# Argument parsing
parser = argparse.ArgumentParser(description="Process inputs to delete/deploy PCD SaaS KDU.")
parser.add_argument('--customer', '-c', required=False, help='Name of the CUSTOMER')
parser.add_argument('--get-kubeconfig', required=False, help='set access and fetch kubeconfig of speecified cloud space', default='')
...
parser.add_argument('--infra', action='store_true', help='Use infra payload create/delete commands')
parser.add_argument('--region', action='store_true', help='Use region payload and create/delete commands')

parser.add_argument('--delete', required=False, help='Set to "yes" to run delete request', default='no')
parser.add_argument('--create', required=False, help='Set to "yes" to run delete request', default='no')
...
if DO_DELETE:
    ...
    if USE_INFRA:
        ...
        print('Delete infra region response:',response_infra.status_code, response_infra.text)
        ...
        response_email = requests.request('DELETE', email_url, json=customer_email)
        print('Remove customer response response:', response_email.status_code, response_email.text)
        ...
        response_region = requests.request('BURN', region_url, data=deploy_data_region, headers=headers)
        print('Delete region response:',response_region.status_code, response_region.text)
        print('Deleting region customer:', customer_url_region)
        response_customer_region = requests.request('DELETE', customer_url_region, headers=headers)
...
if DO_CREATE:
    ...
    if USE_INFRA:
        ...
        response_email = requests.post(url_email, json={"admin_email": env['EMAIL']})
        ...
        response_deploy = requests.request('DEPLOY', url_deploy, data=deploy_data_infra, headers=headers)
        print('Deploy infra region response:', response_deploy.status_code, response_deploy.text)
        ...
        response_deploy = requests.request('DEPLOY', url_deploy, data=deploy_data_region, headers=headers)
        print('Deploy region response:', response_deploy.status_code, response_deploy.text)