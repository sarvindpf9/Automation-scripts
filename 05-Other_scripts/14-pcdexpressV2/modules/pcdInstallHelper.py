...
# Render vars from CSV file
def render_vars_csv(context, template_path, template_file, output_path):
    try:
        ...
        exit(1)
    print(f"Vars file written to: {output_path}")
...
def build_inventory_from_csv(csv_file, env_data=None):
    regions = {}
    all_vars = {
        'pcd': {
            'cloud': env_data.get('portal', '') if env_data else '',
            'url': env_data.get('url', '') if env_data else ''
        }
    }
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            env = row['environment']
            ip = row['ip']
            ssh_user = row.get('ansible_ssh_user')
            ssh_key = row.get('ansible_ssh_key')
            roles = [r.strip() for r in row['roles'].split(',')
                     ] if row.get('roles') else []
            hostcluster = row.get('hostcluster')
            hostconfigs = row.get('hostconfigs', '')
            persistent_backends = [b.strip() for b in row.get('persistent_storage_backends', '').split(
                ',')] if row.get('persistent_storage_backends') else []
            ...
            if env not in regions[region]['environments']:
            ...
            if 'hypervisor' in roles:
                host_data['hostcluster'] = hostcluster
            if 'persistent-storage' in roles:
                host_data['persistent_storage'] = {
                    'backends': persistent_backends}

            regions[region]['environments'][env]['hosts'][ip] = host_data
