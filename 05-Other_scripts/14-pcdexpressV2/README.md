...
- [Setup](#setup)
  ...
  - [Resource configuration references](#resource-configuration-references)
    ...
    - [4. Host/node onboarding under DU management](#4-hostnode-onboarding-under-du-management)
      - [Option1: Onboarding host using the a node config `csv` file](#option1-onboarding-host-using-the-a-node-config-csv-file)
      - [Option2: Onboarding host via the `nodesdata.yaml` modification](#option2-onboarding-host-via-the-nodesdatayaml-modification)
...
- [Quick command references](#quick-command-references)
  ...
  - [Onboarding and assigning Roles to nodes For SaaS based deployments](#onboarding-and-assigning-roles-to-nodes-for-saas-based-deployments)
    - [Using Option1 with `csv` file](#using-option1-with-csv-file)
    - [Using Option2 with `nodesdata.yaml` file](#using-option2-with-nodesdatayaml-file)
  - [Onboarding and assigning Roles to nodes For On-prem based deployments](#onboarding-and-assigning-roles-to-nodes-for-on-prem-based-deployments)
    - [Using Option1 with `csv` file](#using-option1-with-csv-file-1)
    - [Using Option2 with `nodesdata.yaml` file](#using-option2-with-nodesdatayaml-file-1)
...
-  Inside the repo run the `setup-local.sh` script which will install and ocnfigure the require packages and libaries.
  
All the required dependencies, including the Ansible binaries and PCD Collection will be available at this point.

- Run the provided command to initialise the template configurations

```bash
./pcdExpress -portal <region> -region <location> --url "<URL Of the DU portal>" --env <env_type> --setup-environment yes
...
```
prod:
  url: "https://acme-nyc.app.qa-pcd.platform9.com/" <--- region URL
  cloud: "region2"
  blueprints:
    region2:
      name: region2_demo
      ...
      dnsDomainName: region2.local
      virtualNetworking:
        enabled: true
        underlayType: geneve
        vnidRange: '1:65535'
      imageLibraryStorage: "NFS_glance"
      imageLibrarySharedStorage: true
...
For onboarding there are currently **two different methods** to do that. One can choose any one of the below method based on the requirements.

##### Option1: Onboarding host using the a node config `csv` file 
- Copy the `nodeconfig.csv-sample` into `nodeconfig.csv`
- Prepare the CSV file with required node data with the roles. A Sample file is as below:
```
region,environment,ip,ansible_ssh_user,ansible_ssh_key,roles,hostcluster,persistent_storage_backends,hostconfigs
region2,develop,172.29.20.37,ubuntu,/home/ubuntu/.ssh/id_rsa,"persistent-storage,image-library,hypervisor",cluster-4,"nfs_cinder_backend,nfs_glance_backend",rspc_218_net
region2,develop,172.29.20.59,ubuntu,/home/ubuntu/.ssh/id_rsa,"hypervisor",cluster-4,,rspc_218_net
```
- Run the express `--create-hostagents-configs yes` command with the `--csv filename.csv`  flag:
```
./pcdExpress  -env-file user_configs/sa-demo/region2/sa-demo-region2-develop-environment.yaml  --csv nodeconfig.csv --create-hostagents-configs yes
cloud region credential directory present. proceeding...
Generating inventory files..
Generating hosts inventory from CSV file..
Vars file written to: sa-demo-play_data/region2/inventory/develop/hosts
vars file generated successfully and saved at location:
sa-demo-play_data/region2/playbooks/sa-demo-region2-develop-node-onboard.yaml
```
**NOTE**: 
1. The name of the csv file can be anything, this example takes up the name as `nodeconfig.csv`
2. **If the roles section is undefined, the hosts will be onboarded in un-authorised state**

- Run the apply command command:
```
./pcdExpress -env-file user_configs/sa-demo/region2/sa-demo-region2-develop-environment.yaml  --apply-hosts-onboard yes
cloud region credential directory present. proceeding...
User defined templates directory found at user_configs
User defined region templates directory found..
User defined site templates directory found..
...
TASK [Get Auth Token] **********************************************************
ok: [localhost] => {"auth_token": "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9..dxZBRET_rIp1OVDVFOYccWiLCJ8kyTDHHsOWhd6sOyhIWCmOTwjXa85Y2ogYD79d8DXKk01dGQRsXeq-JTCnFQ", "changed": false}

TASK [set_fact] ****************************************************************
ok: [localhost] => {"ansible_facts": {"keystone_token": "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9..dxZBRET_rIp1OVDVFOYccWiLCJ8kyTDHHsOWhd6sOyhIWCmOTwjXa85Y2ogYD79d8DXKk01dGQRsXeq-JTCnFQ"}, "changed": false}
...
TASK [process excluded roles as input] *****************************************
ok: [172.29.20.37] => {"ansible_facts": {"excluded_roles": []}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"excluded_roles": []}, "changed": false}

TASK [Get roles for servers] ***************************************************
ok: [172.29.20.37] => {"ansible_facts": {"rolenames": ["persistent-storage", "image-library", "hypervisor"]}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"rolenames": ["hypervisor"]}, "changed": false}

TASK [export network hostconfigs names] ****************************************
ok: [172.29.20.37] => {"ansible_facts": {"hostconfig": "rspc_218_net"}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"hostconfig": "rspc_218_net"}, "changed": false}

TASK [set_fact] ****************************************************************
ok: [172.29.20.37] => {"censored": "the output has been hidden due to the fact that 'no_log: true' was specified for this result", "changed": false}
ok: [172.29.20.59] => {"censored": "the output has been hidden due to the fact that 'no_log: true' was specified for this result", "changed": false}

TASK [set_fact] ****************************************************************
ok: [172.29.20.37] => {"ansible_facts": {"cloud_name": "Region2"}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"cloud_name": "Region2"}, "changed": false}

TASK [set_fact] ****************************************************************
ok: [172.29.20.37] => {"ansible_facts": {"filtered_roles": ["persistent-storage", "image-library", "hypervisor"]}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"filtered_roles": ["hypervisor"]}, "changed": false}

TASK [Include roles for the nodes] *********************************************
included: persistent-storage for 172.29.20.37 => (item=persistent-storage)
included: image-library for 172.29.20.37 => (item=image-library)
included: hypervisor for 172.29.20.37, 172.29.20.59 => (item=hypervisor)

TASK [persistent-storage : shell] **********************************************
changed: [172.29.20.37] => {"ansible_facts": {"discovered_interpreter_python": "/usr/bin/python3.10"}, "changed": true, "cmd": "sudo grep host_id /etc/pf9/host_id.conf | cut -d '=' -f2 | sed -e 's/ //g'", "delta": "0:00:00.016380", "end": "2025-08-04 11:11:38.500532", "msg": "", "rc": 0, "start": "2025-08-04 11:11:38.484152", "stderr": "", "stderr_lines": [], "stdout": "9961738e-5284-439d-a1c0-99f8681994f5", "stdout_lines": ["9961738e-5284-439d-a1c0-99f8681994f5"]}

TASK [persistent-storage : Set fact for Host ID] *******************************
ok: [172.29.20.37] => {"ansible_facts": {"hostid": "9961738e-5284-439d-a1c0-99f8681994f5", "tags": "hostid"}, "changed": false}

TASK [persistent-storage : Display Host ID] ************************************
ok: [172.29.20.37] => {
    "hostid": "9961738e-5284-439d-a1c0-99f8681994f5"
}

TASK [persistent-storage : Set PCD Roles] **************************************
changed: [172.29.20.37] => {"changed": true, "msg": "Role 'persistent-storage' is already applied. No changes made", "new_config": "[\"persistent-storage\"]"}

TASK [image-library : shell] ***************************************************
changed: [172.29.20.37] => {"changed": true, "cmd": "sudo grep host_id /etc/pf9/host_id.conf | cut -d '=' -f2 | sed -e 's/ //g'", "delta": "0:00:00.014044", "end": "2025-08-04 11:11:43.250938", "msg": "", "rc": 0, "start": "2025-08-04 11:11:43.236894", "stderr": "", "stderr_lines": [], "stdout": "9961738e-5284-439d-a1c0-99f8681994f5", "stdout_lines": ["9961738e-5284-439d-a1c0-99f8681994f5"]}

TASK [image-library : Set fact for Host ID] ************************************
ok: [172.29.20.37] => {"ansible_facts": {"hostid": "9961738e-5284-439d-a1c0-99f8681994f5", "tags": "hostid"}, "changed": false}

TASK [image-library : Display Host ID] *****************************************
ok: [172.29.20.37] => {
    "hostid": "9961738e-5284-439d-a1c0-99f8681994f5"
}

TASK [image-library : Set PCD Roles] *******************************************
changed: [172.29.20.37] => {"changed": true, "msg": "Role 'image-library' is already applied. No changes made", "new_config": "[\"image-library\"]"}

TASK [hypervisor : shell] ******************************************************
changed: [172.29.20.37] => {"changed": true, "cmd": "sudo grep host_id /etc/pf9/host_id.conf | cut -d '=' -f2 | sed -e 's/ //g'", "delta": "0:00:00.016202", "end": "2025-08-04 11:11:45.113155", "msg": "", "rc": 0, "start": "2025-08-04 11:11:45.096953", "stderr": "", "stderr_lines": [], "stdout": "9961738e-5284-439d-a1c0-99f8681994f5", "stdout_lines": ["9961738e-5284-439d-a1c0-99f8681994f5"]}
changed: [172.29.20.59] => {"ansible_facts": {"discovered_interpreter_python": "/usr/bin/python3.10"}, "changed": true, "cmd": "sudo grep host_id /etc/pf9/host_id.conf | cut -d '=' -f2 | sed -e 's/ //g'", "delta": "0:00:00.013678", "end": "2025-08-04 11:11:45.164388", "msg": "", "rc": 0, "start": "2025-08-04 11:11:45.150710", "stderr": "", "stderr_lines": [], "stdout": "3a1c68cf-2fa0-4d86-82fa-02ef1eb53d73", "stdout_lines": ["3a1c68cf-2fa0-4d86-82fa-02ef1eb53d73"]}

TASK [hypervisor : Set fact for Host ID] ***************************************
ok: [172.29.20.37] => {"ansible_facts": {"hostid": "9961738e-5284-439d-a1c0-99f8681994f5", "tags": "hostid"}, "changed": false}
ok: [172.29.20.59] => {"ansible_facts": {"hostid": "3a1c68cf-2fa0-4d86-82fa-02ef1eb53d73", "tags": "hostid"}, "changed": false}
...
ok: [172.29.20.59] => {
    "hostid": "3a1c68cf-2fa0-4d86-82fa-02ef1eb53d73"
}

TASK [hypervisor : Set PCD Roles] **********************************************
ok: [172.29.20.59] => {"changed": false, "msg": "Host already in cluster 'cluster-4'. Skipped.", "new_config": "No changes"}
ok: [172.29.20.37] => {"changed": false, "msg": "Host already in cluster 'cluster-4'. Skipped.", "new_config": "No changes"}

PLAY RECAP *********************************************************************
172.29.20.37               : ok=21   changed=5    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
172.29.20.59               : ok=11   changed=1    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
localhost                  : ok=6    changed=1    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
```

##### Option2: Onboarding host via the `nodesdata.yaml` modification
...
#### Using Option1 with `csv` file
**Total steps needed**: **03**
- Create a `csv` file with the need host details and respective roles
- Run the `--create-hostagents-configs` with `--csv nodes.csv` option to create the inventory and playbooks
...
#### Using Option2 with `nodesdata.yaml` file 
**Total steps needed**: **04**
- Modify the file in `user_configs/node_onboarding/`
...
**Pre-requistes**:
- `nodes.csv` file should be generated and populated with necessary details
- Any changes to nodes data, only this file should be modified
